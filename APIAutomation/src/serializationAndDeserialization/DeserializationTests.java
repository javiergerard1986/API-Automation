...
public class DeserializationTests {
	...
	public void getCode(){
		...
		codeValue = "4%2F0AX4XfWj5mOIqcM8Pnr2bufG1a3unHUQcvIvfxaVxc6Qx8EhqpIcOZCPK9WIbHgl_5QnV9g";
	}
	...
	public void getCourses(){
		...
		// Get courses
		RestAssured.baseURI = baseUrl;
		GetCoursesResponse response = given().queryParam(accessTokenQParam, accessTokenValue).expect().defaultParser(Parser.JSON)
		.when().get(getCourse).as(GetCoursesResponse.class);
		
		// Print Instructor name and instructor linkedin
		System.out.println("Instructor Name: " + response.getInstructor());
		System.out.println("Instructor LinkedIn: " + response.getLinkedIn());
		
		// Print price of the course SoapUI Webservices testing that is under the API main course
		for (Map.Entry<String, List<SubCourse>> entry : response.getCourses().entrySet()) {
		    if(entry.getKey().equals("api")) {
		    	for(int x = 0; x < entry.getValue().size(); x++) {
		    		if(entry.getValue().get(x).getCourseTitle().equals("SoapUI Webservices testing")) {
		    			System.out.println("Course Area: " + entry.getKey());
		    			System.out.println("Course Name: " + entry.getValue().get(x).getCourseTitle());
		    			System.out.println("Course Price: " + entry.getValue().get(x).getPrice());
		    		}
		    	}
		    }
		...
		// Print the list of subcourses that relates to the webAutomation area, and the sum of the price for all the courses of the area
		int areaPriceAmount = 0;
		for (Map.Entry<String, List<SubCourse>> entry : response.getCourses().entrySet()) {
		    if(entry.getKey().equals("webAutomation")) {
		    	System.out.println("-----------------------");
		    	System.out.println("Course Area: " + entry.getKey());
    			System.out.println("-----------------------");
		    	for(int x = 0; x < entry.getValue().size(); x++) {
		    		...
		    		System.out.println("Course Name: " + entry.getValue().get(x).getCourseTitle());
		    		System.out.println("Course Price: " + entry.getValue().get(x).getPrice());
		    		areaPriceAmount += Integer.parseInt(entry.getValue().get(x).getPrice());
		    	}
		    	System.out.println("-----------------------");
		    	System.out.println("Price of all courses inside the " + entry.getKey() + " area: " + areaPriceAmount);
    			System.out.println("-----------------------");
		    }
		}
		
		Assert.assertEquals(areaPriceAmount, 130);
